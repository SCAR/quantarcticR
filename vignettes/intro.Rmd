...
```{r}
icethickness <- qa_get("BEDMAP2 Ice thickness uncertainty", verbose = TRUE)
class(icethickness)
```

This returns a object of class `r class(icethickness)`.
...
```{r}
raster::plot(icethickness)
```

# Using datasets with sf

```{r}
surface_sf <- qa_get("BEDMAP2 Surface elevation (1km)", shapefile_reader = sf::st_read)
surface_sf
class(surface_sf)
##surface_sf %>% 
##  ggplot2::ggplot() +
